  ...
  if (!product) {
    return (
      <main className="pt-[120px] pb-xl max-w-container-max mx-auto px-gutter text-center">
        <div className="glass-panel max-w-md mx-auto p-8 rounded-3xl bg-white/80 border border-white/80">
          <span className="material-symbols-outlined text-5xl text-stone-400 mb-2">search_off</span>
          <h1 className="font-headline text-2xl font-bold text-on-surface">Product Not Found</h1>
          <p className="text-sm text-stone-500 mt-2 mb-6">The requested luxury grocery item could not be found.</p>
  ...
  return (
    <main className="pt-6 md:pt-8 pb-xl max-w-container-max mx-auto px-margin-mobile md:px-gutter">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-16">
        {/* Left Column: Image Gallery & Origin Specs */}
        <div className="lg:col-span-7 flex flex-col gap-4">
          <div className="w-full aspect-[4/3] md:aspect-[16/11] rounded-3xl overflow-hidden glass-panel relative group border border-white/80 bg-stone-100 shadow-md">
            <img
            ...
            <div className="absolute top-4 left-4 flex flex-col gap-2">
              {product.badge && (
                <div className="px-3.5 py-1 bg-white/85 backdrop-blur-md border border-white/80 rounded-full text-xs font-bold uppercase tracking-wider text-primary shadow-sm">
                  {product.badge}
                </div>
              )}
              {product.season && (
              ...
              {discountPercent && (
                <div className="px-3 py-1 bg-rose-500 text-white rounded-full text-xs font-extrabold shadow-sm">
                  {discountPercent}% OFF
                </div>
              )}
            </div>
          ...
          <div className="glass-panel p-6 rounded-3xl border border-white/80 space-y-3 bg-white/75 shadow-sm">
            ...
            <p className="text-xs text-stone-600">
              <strong>Source Farm / Estate:</strong> {product.origin || "Lumina Certified Heirloom Estate"}
            </p>
            ...
            {product.nutrition && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2">
                {Object.entries(product.nutrition).map(([key, val]) => (
                  <div key={key} className="bg-white/60 p-2.5 rounded-xl border border-white/70 text-center shadow-xs">
                    <p className="text-[10px] uppercase font-bold text-stone-500">{key}</p>
        ...
        {/* Right Column: Sticky Product Buy Box */}
        <div className="lg:col-span-5 sticky top-28">
          <div className="glass-panel rounded-3xl p-6 md:p-8 flex flex-col gap-6 border border-white/85 ambient-shadow bg-white/80">
            {/* Header info */}
            <div>
              <div className="flex items-center gap-2 text-xs text-amber-700 mb-2">
                <span className="material-symbols-outlined text-base text-amber-500 fill-current">star</span>
                <span className="font-bold text-on-surface">{product.rating || 4.9}</span>
                <span className="text-stone-500">({product.reviewsCount || 100} certified reviews)</span>
                <span className="ml-auto text-emerald-700 font-semibold bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200 text-[11px]">
                  In Stock & Ready
                </span>
              </div>
            ...
            <div className="pt-4 border-t border-stone-200/60 flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-stone-500">Select Quantity</span>
              <div className="flex items-center gap-4 bg-white/70 backdrop-blur-md rounded-full px-4 py-1.5 border border-stone-200 shadow-xs">
                <button
                  onClick={() => setQty(Math.max(1, qty - 1))}
                  className="w-7 h-7 rounded-full flex items-center justify-center text-on-surface hover:bg-stone-100 transition-colors"
                >
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 text-[11px] text-stone-500">
              <div className="flex items-center gap-2">
                ...
                <span>Express Eco Delivery</span>
              </div>
              <div className="flex items-center gap-2">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h2 className="font-headline text-2xl font-bold text-on-surface">Complementary Pairings</h2>
            </div>
            <Link to={`/search?q=${encodeURIComponent(product.category)}`} className="text-primary text-xs font-bold hover:underline">
